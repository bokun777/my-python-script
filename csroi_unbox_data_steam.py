...
# Scraper Average Unbox (USD) i Unbox ROI (%) z CSROI (pricing source: Steam – ustaw ręcznie przy 1. runie)
# - persistent profile: playwright_profile_csroi_steam
# - zapis: data/csroi_unbox_data_steam.ndjson
...
PROFILE_DIR = Path("playwright_profile_csroi_steam")
OUTFILE = Path("data/csroi_unbox_data_steam.ndjson")
...
def pretty_name_from_slug(slug: str) -> str:
    ...
    # domyślne formatowanie: "chroma-2-case" -> "Chroma 2 Case"
...
def main():
    parser = argparse.ArgumentParser(description="CSROI Steam – Average Unbox + Unbox ROI scraper (cookies profile)")
    parser.add_argument("--visible", action="store_true", help="Otwórz okno i ręcznie ustaw Pricing source: Steam (pierwszy run)")
    ...
    with sync_playwright() as p:
        ...
        # Pierwszy run: ustaw Pricing source: Steam i zaakceptuj cookies (jeśli trzeba)
        if args.visible:
            page.goto("https://csroi.com/item/horizon-case", wait_until="domcontentloaded", timeout=60000)
            print("➡️ Ustaw na stronie Pricing source: Steam (na górze strony / w menu).")
            input("Gdy ustawisz Steam i zaakceptujesz cookies, wciśnij ENTER tutaj...")
